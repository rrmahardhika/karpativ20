    ...
    <v-container style="padding:40px" justify-center fluid>
      <v-layout mb-5>
        ...
        <v-flex class="text-md-right pt-3">
          <v-btn class="myBtn" to="assets/create" depressed dark color="rgba(255,101,57,1)">Entri</v-btn>
        </v-flex>
      </v-layout>
      <v-layout mt-5 pt-5>
          ...
          >
            <template v-slot:item="{ item, index }">
              <tr>
                ...
                <td>{{getDisplayDate(item.tanggal_perolehan)}}</td>
                <td class="text-right">{{formatMoney(item.nilai_perolehan)}}</td>
                <td class="text-center">{{item.lokasi}}</td>
                <td class="text-center">
                  <template v-if="item.kondisi===1">
                    <v-chip color="green" text-color="white" small>Bagus</v-chip>
                  </template>
                  <template v-else-if="item.kondisi===2">
                    <v-chip color="red" text-color="white" small>Rusak</v-chip>
                  </template>
                  <template v-else-if="item.kondisi===3">
                    <v-chip color="orange" text-color="white" small>Sedang Diperbaiki</v-chip>
                  </template>
                  <template v-else-if="item.kondisi===4">
                    <v-chip color="secondary" text-color="black" small>Hilang</v-chip>
                  </template>
                </td>
                <td>
                  <v-btn
                    ...
                    color="rgba(255,101,57,1)"
                    class="myBtn2"
                    @click="$router.push('assets/edit/'+item.id)"
                  >Edit</v-btn>&nbsp;
                  <v-btn
                    outlined
                    depressed
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Kondisi", value: "kondisi", align: "center" },
