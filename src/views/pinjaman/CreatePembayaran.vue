    ...
    <v-container style="padding:40px" justify-center fluid>
      <v-layout mb-5>
        ...
        <v-flex class="text-md-right pt-3">
          ...
          >Kembali ke Detail Pinjaman</v-btn>
        </v-flex>
      </v-layout>
      <v-layout mt-5 pt-5>
        <v-flex xs6 class="tableContent">
            ...
            <v-layout wrap mb-2 style="font-size:14px;">
              ...
              <v-flex sm4 xs12>
                <v-menu
                  ...
                  :return-value.sync="newData.tgl_pembayaranpinjaman"
                ...
                >
                  ...
                  <v-date-picker
                    v-model="newData.tgl_pembayaranpinjaman"
                    locale="id-id"
                    color="rgba(255,101,57,1)"
                    scrollable
                    @change="$refs.menu_tanggal.save(newData.tgl_pembayaranpinjaman);"
                  ></v-date-picker>
                </v-menu>
              </v-flex>
            </v-layout>
          ...
          <v-layout>
           ...
           <v-flex v-else-if="computed_sisabayar<0" style="font-size:14px; font-style:italic; color:red">Total Tagihan hanya <span style="font-weight:600;">{{formatMoney(detailData.ttl_pinjam-detailData.ttl_pembayaran)}} - Pembayaran Berlebih</span></v-flex>
...
export default {
  ...
  computed: {
    computed_tanggal() {
      return this.newData.tgl_pembayaranpinjaman
        ? moment(this.newData.tgl_pembayaranpinjaman).format("DD MMMM YYYY")
        : "";
    },
    computed_sisabayar(){
      return this.detailData.ttl_pinjam-this.detailData.ttl_pembayaran-this.newData.nominal;
    }
  },
  methods: {
    getById() {
    ...
    initNew() {
      this.newData = {
        tgl_pembayaran: "",
        nominal: 0,
        id_pinjaman:this.$route.params.id
      };
    },
    convertDate(tgl) {
