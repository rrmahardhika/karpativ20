    ...
    <v-container style="padding:40px" justify-center fluid>
      <v-layout mb-5>
        <v-flex class="pl-5 ml-1">
          <div class="myTitle">Detail Pembelian</div>
          <div class="breadcums">
      ...
      <v-layout mt-5 pt-5>
        <v-flex xs6 class="tableContent">
          <v-layout mb-3 pt-1>
            <div class="myTitle" style="font-size:20px">DATA PEMBELIAN</div>
          </v-layout>
          <v-layout wrap mb-2 style="font-size:14px;">
            <v-layout wrap mb-2 style="font-size:14px;">
              <v-flex xs12 style="font-weight:600;" class="mb-1">Nomor Pembelian</v-flex>
              <v-flex sm5 xs12 class="detailPenjualan">{{detailData.no_pembelian}}</v-flex>
            </v-layout>
            <v-flex xs12 style="font-weight:600;" class="mb-1">Tanggal Pembelian</v-flex>
            <v-flex xs12 class="detailPenjualan">{{convertDate(detailData.tgl_pembelian)}}</v-flex>
          </v-layout>
          <v-layout wrap mb-2 style="font-size:14px;">
            <v-flex xs12 style="font-weight:600;" class="mb-1">Supplier</v-flex>
            <v-flex xs12 class="detailPenjualan">{{detailData.nama_supplier}}</v-flex>
          </v-layout>
          <v-layout wrap mb-2 style="font-size:14px;">
            ...
            <v-flex xs12 class="detailPenjualan">
              ...
              <v-chip v-else-if="detailData.ttl_pembelian-detailData.ttl_pembayaran>0" color="warning">Sedang diangsur</v-chip>
              <v-chip v-else-if="detailData.ttl_pembelian-detailData.ttl_pembayaran==0" color="success">Lunas</v-chip>
            </v-flex>
          </v-layout>
        </v-flex>
      </v-layout>
      <v-layout mt-2 pt-5>
        <v-flex xs12 class="tableContent">
          <v-layout mb-3 pt-1>
            <v-flex>
              <div class="myTitle" style="font-size:20px">ITEM PEMBELIAN</div>
            </v-flex>
          </v-layout>
          <v-data-table
            :headers="headers"
          ...
          >
            ...
            <template v-slot:body.append>
              <tr>
                ...
                >
                  <v-flex pt-4>{{formatMoney(detailData.ttl_pembelian)}}</v-flex>
                </td>
              </tr>
            </template>
          </v-data-table>
      ...
      <v-layout mt-2 pt-5>
        <v-flex xs12 class="tableContent">
          <v-layout mb-3 pt-1>
            <v-flex>
              <div class="myTitle" style="font-size:20px">PEMBAYARAN PEMBELIAN</div>
            </v-flex>
            <v-flex v-if="detailData.ttl_pembelian-detailData.ttl_pembayaran>0" class="text-right">
              <v-btn
                class="myBtn noActive"
                depressed
                dark
                color="rgba(255,101,57,1)"
                    @click="$router.push('/pembelian/detail/pembayaran/'+$route.params.id)"
          ...
          >
            <template v-slot:item="{ item, index }">
              <tr>
                ...
                <td>
                  <v-layout wrap mb-2 style="font-size:14px;">
                    <v-flex xs12 pt-2>{{convertDate(item.tgl_pembayaranpembelian)}}</v-flex>
                  </v-layout>
                </td>
            ...
            <template v-slot:body.append>
              ...
              <tr>
                ...
                <td
                  ...
                  style="text-transform:none; font-size:20px;"
                >
                  <v-flex colspan="2" pt-4>{{formatMoney(detailData.ttl_pembelian-detailData.ttl_pembayaran)}}</v-flex>
                </td>
              </tr>
            </template>
          </v-data-table>
...
export default {
  ...
  computed: {
    computed_tanggal() {
      return this.newData.tgl_pembelian
        ? moment(this.newData.tgl_pembelian).format("DD MMMM YYYY")
        : "";
    }
  },
  methods: {
