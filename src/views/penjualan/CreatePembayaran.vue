      ...
      <v-layout mb-5>
        ...
        <v-flex class="text-md-right pt-3">
          <v-btn
            ...
            @click="$router.push('/penjualan/detail/'+$route.params.id)"
            depressed
            outlined
            dark
            color="rgba(255,101,57,1)"
          >Kembali ke Detail Penjualan</v-btn>
        </v-flex>
      </v-layout>
      <v-layout mt-5 pt-5>
            ...
            <v-layout wrap mb-2 style="font-size:14px;">
              ...
              <v-flex sm4 xs12>
                <v-menu
                  ...
                  :return-value.sync="newData.tgl_pembayaranpenjualan"
                ...
                >
                  ...
                  <v-date-picker
                    v-model="newData.tgl_pembayaranpenjualan"
                    locale="id-id"
                    color="rgba(255,101,57,1)"
                    scrollable
                    @change="$refs.menu_tanggal.save(newData.tgl_pembayaranpenjualan);"
                  ></v-date-picker>
                </v-menu>
              </v-flex>
            </v-layout>
...
export default {
  ...
  computed: {
    computed_tanggal() {
      return this.newData.tgl_pembayaranpenjualan
        ? moment(this.newData.tgl_pembayaranpenjualan).format("DD MMMM YYYY")
        : "";
    },
    computed_sisabayar(){
      return this.detailData.ttl_penjualan-this.detailData.ttl_pembayaran-this.newData.nominal;
    }
  },
  methods: {
    getById() {
