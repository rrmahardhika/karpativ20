  ...
  <v-content class="content">
    <v-container style="padding:40px" justify-center fluid>
      <v-layout mb-5>
        <v-flex class="pl-5 ml-1">
          <div class="myTitle">Penjualan</div>
          <div class="breadcums">
        ...
        <v-flex class="text-md-right pt-3">
          <v-btn class="myBtn" to="penjualan/create" depressed dark color="rgba(255,101,57,1)">Entri</v-btn>
        </v-flex>
      </v-layout>
      <v-layout mt-5 pt-5>
          ...
          >
            <template v-slot:item="{ item, index }">
              <tr>
                ...
                <td>{{convertDate(item.tgl_penjualan)}}</td>
                <td>{{item.nama_anggota}}</td>
                <td>{{item.ttl_penjualan}}</td>
                <td>
                  <v-chip small v-if="item.ttl_pembayaran<=0" color="error">Belum dibayar</v-chip>
                  <v-chip small v-else-if="item.ttl_penjualan-item.ttl_pembayaran>0" color="warning">Sedang diangsur</v-chip>
                  <v-chip small v-else-if="item.ttl_pembayaran===item.ttl_penjualan"  color="success">Lunas</v-chip>
                </td>
                <td class="text-center">
                  <v-btn
                    ...
                    color="rgba(255,101,57,1)"
                    class="myBtn2"
                    @click="$router.push('penjualan/detail/'+item.id)"
                  >Detail</v-btn>&nbsp;
                  <v-btn
                    outlined
                    depressed
    ...
    <v-dialog v-model="dialogHapus" max-width="290">
      <v-card class="pa-1">
        ...
        <v-card-text>Yakin hapus penjualan ini?</v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn
            depressed
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Tanggal", value: "tgl_penjualan" },
        { text: "Nama Anggota", value: "nama_anggota" },
        { text: "Total Penjualan", value: "ttl_penjualan" },
        { text: "Status Bayar", value: "status" },
        { text: "Aksi", value: "aksi", sortable: false, align: "center" }
      ],
      items: [],
      dialogHapus: false,
      sortTable: "tgl_penjualan",
      anggotaChoice: []
    };
  },
  computed: {
